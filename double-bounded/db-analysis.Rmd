...
p_rmse_showcase <- df %>%
  ...
  summarise(mean_rmse_s = median(rmse_s),
  ...
  #coord_cartesian(xlim = c(0, 0.5)) +
  #scale_x_continuous(labels = c("0", "0.25", "0.5"), breaks = c(0, 0.25, 0.5)) +
  geom_pointrange(size = 0.4, position = position_dodge(width = 0.9)) +
  #geom_point(position = position_dodge(width = 0.9), size = 4 / 3^0.25) +
...
p_rmse_showcase
ggsave(paste0(figure_path,"unit_rmse_showcase_pointrange.pdf"), width = 210, height = (297/4)*0.9, units = "mm", useDingbats = TRUE)
...
# Figure to showcase the potential FPR inflation of the normal likelihood
FPR %>%
  filter(fit_family_transformed == "Normal" | fit_family_transformed =="Normal Identity" ) %>%
  filter(!(fit_family_transformed == "Normal Identity" & (fit_link_transformed == "Cauchit" |
                                                          fit_link_transformed == "Cloglog"))) %>%
   mutate(fit_link_transformed = case_when(
     fit_family_transformed == "Normal Identity" ~ "Identity",
     TRUE ~ fit_link_transformed)) %>%
  mutate(fit_link_transformed = factor(fit_link_transformed, levels = c("Logit",
                                           "Cauchit",
                                           "Cloglog",
                                           "Identity"))) %>%
  group_by(fit_link_transformed) %>% 
  mutate(mean = mean(FPR_95)) %>%
  ggplot(aes(FPR_95)) +
  geom_histogram() +
  facet_grid(. ~ fit_link_transformed) +
  geom_vline(xintercept = 0.05) +
  geom_vline(aes(xintercept = mean), linetype = "longdash") +
...
ggsave(paste0(figure_path,"unit_normal_fpr.pdf"), width = 210, height = (297/4)*0.5, units = "mm", useDingbats = TRUE)
...
plot(p2, plot = FALSE)[[1]] + scale_color_scico_d(palette = "batlow", end = 0.8) +
   ...
   theme_bw(base_size = 12) +
        labs(color = "Fit Family", fill = "Fit Family")
ggsave(paste0(figure_path,"unit_auc_cond_fam_fam.pdf"), width = 210, height = (297/4), units = "mm", useDingbats = TRUE)
...
plot(p3, plot = FALSE)[[1]] + xlab("Fit Link") + ylab("AUC") + theme_bw(base_size = 12)
ggsave(paste0(figure_path,"unit_auc_cond_link.pdf"), width = 210/2, height = (297/4), units = "mm", useDingbats = TRUE)
...
plot(p4, plot = FALSE)[[1]] + xlab("Fit Family") + ylab("AUC") + theme_bw(base_size = 12)
ggsave(paste0(figure_path,"unit_auc_cond_fam.pdf"), width = 210/1.2, height = (297/4), units = "mm", useDingbats = TRUE)
...
ggsave(paste0(figure_path, "unit_roc_shape_and_fit.pdf"), width = 210, height = (297/4) * 1.5, units = "mm", useDingbats = TRUE)
```
